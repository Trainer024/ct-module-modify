...
register('chat', (rank, name, mode, floor) => {
	if (!config.partyCommands || Date.now() - lastTimeUsed < 750 || blacklist.igns.includes(name.toLowerCase()) || !Array.from(numbersToText.keys()).includes(floor)) return
...
}).setCriteria(/Party > (\[.+\])? ?(.+) ?[ቾ⚒]?: !(f|m)(\d)/)

register('chat', (rank, name) => {
	if (!config.partyCommands || Date.now() - lastTimeUsed < 750 || blacklist.igns.includes(name.toLowerCase())) return
...
}).setCriteria(/Party > (\[.+\])? ?(.+) ?[ቾ⚒]?: !allinv(ite)?/)
...
}).setCriteria(/From (\[.+\])? ?(.+) ?[ቾ⚒]?: !(party|p|inv|invite)/).setContains()
...
register('chat', (rank, name) => {
	...
	requestedPing = true
	lastTimeUsed = Date.now()
}).setCriteria(/Party > (\[.+\])? ?(.+) ?[ቾ⚒]?: !ping/)

register('chat', (rank, name) => {
	if (!config.partyCommands || Date.now() - lastTimeUsed < 2000 || blacklist.igns.includes(name.toLowerCase())) return
	requestedTPS = true
	lastTimeUsed = Date.now()
}).setCriteria(/Party > (\[.+\])? ?(.+) ?[ቾ⚒]??: !tps/)
